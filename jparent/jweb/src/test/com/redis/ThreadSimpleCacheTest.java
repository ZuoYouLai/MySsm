...
@Slf4j
public class ThreadSimpleCacheTest {
    ...
    public void before() {
        ...
        String key = ToolUtils.getKey(CacheEnum.SIMPLE.getKey(), USER_ID);
    ...
    public void threadTest() {
         ...
         *      在线程池第一次满的时候线程执行结果都为: select
         *      在第二次执行的任务的都为: cache
         *
         * 说明:
         * 并发执行时，会出现redis查无值，并且直接查库操作,导致数据库有直接压力
         */
    }
    ...
    private class SelectTask implements Runnable {
        @Override
        public void run() {
            userCacheService.simpleUserCache(USER_ID);
